                    ...
                    <div class="product-order">
                        ...
                        @foreach($order->products()->get() as $item)
                            ...
                            <div class="row product-order-detail">
                                <div class="col-2"><img style="width: 100px"
                        ...
                        <div class="total-sec">
                            <ul>
                                <li>{{__('titles.sub_total_price')}}
                                    <span>${{number_format($order->billing_subtotal)}}</span></li>
                        ...
                        <div class="final-total m-t-10">
                            <h3>{{__('titles.total_price')}} <span>${{number_format($order->billing_total)}}</span></h3>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    ...
                    <div class="bg-light m-t-10">
                        <form action="{{route('seller.orders.change_status', $order->id)}}" class="form-control"
                              method="post">
                            @csrf @method('patch')
                            ...
                            <select class="form-control" name="condition">
                                @foreach(\App\Models\Order::conditionValues() as $condition)
                                    <option value="{{$condition}}"
                                        {{$order->condition == $condition ? 'selected' : ''}}
                                        {{$order->condition == 'cancelled' && $condition == 'completed' ? 'disabled' : ''}}
                                        {{$order->condition == 'completed' && $condition == 'cancelled' ? 'disabled' : ''}}
                                        {{($order->condition == 'completed' ||  $condition == 'cancelled' ) && ($condition == 'processing' || $condition == 'pending') ? 'disabled' : ''}}

                                    >
                                        {{__("validation.attributes.$condition")}}
                                    </option>
                                @endforeach
                            </select>
